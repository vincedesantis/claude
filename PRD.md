...
**Definition of done for v1:** Vince adds a ticker, the system silently monitors it, and daily at market-close-plus-thirty he gets one email that fully replaces checking any other source for that period.
...
- **No real-time push notifications.** Digest only, daily at the fixed time in Section 5.1 — this is the product's core value prop, not a missing feature.
...
- **No cadence choice, per-company or per-account.** Every account gets one digest a day, at the same fixed time as every other account (Section 5.1) — no configurability here at all, not even the daily/weekly/biweekly/monthly account-level knob v1 originally had.
## 5. Core Mechanism
**Watch → Detect → Filter → Summarize → Batch → Send.**
Monitoring and the digest send both run **daily** (market days only) for every account, at the same two fixed times for everyone (Section 5.1) — no per-user schedule of any kind.
...
- **Digest cadence:** removed entirely (was daily/weekly/biweekly/monthly, one setting per account) — every account now gets exactly one digest per market day, no per-account or per-company choice. Fixed schedule for everyone, no user-facing configurability: monitoring at market close (1pm Pacific), digest 90 minutes later at 2:30pm Pacific (`vercel.json`). Simpler than supporting per-user send times, which Vercel Cron's static schedule model doesn't cleanly support anyway (would need either multiple fixed cron triggers or a much more frequent polling cron plus a stored per-user timezone).
...
- No cadence setting — every account gets a daily digest at the fixed time in Section 5.1, not configurable per account (Section 4).
...
- Every market day: pull all unsent items, group by company, render as HTML email, send, mark items sent. No cadence gate — see Section 5.1.
...
- **Digest preview:** `GET /api/preview-digest` renders exactly what the signed-in user's next real send would contain, without sending anything or marking items as sent. Bypasses the weekend gate, since the point is to preview regardless of whether today is actually a market day.
...
- **Cron routes loop over every account** (`listAllUsers()`) instead of the single implicit user — each account's watchlist is monitored, and each gets their own digest sent to their own account email, independently of every other account's failures.
## 7. API Contracts
...
GET|POST /api/cron/digest         // triggered by Vercel Cron, daily at 2:30pm Pacific — no cadence check, every market day is a send day
  headers: { "x-cron-secret": "..." }  // or Authorization: Bearer <CRON_SECRET>
  200:  { "sent": true, "item_count": 5 }
  200:  { "sent": false, "reason": "market closed (weekend)" }
  500:  { "error": "digest run failed" }   // also triggers a failure alert email (Section 6.5)
```
## 8. Data Model
```
users
  id, email, created_at, auth_user_id (nullable, unique, references auth.users)
  ...
  -- digest_cadence removed (supabase/migrations/0004_remove_cadence.sql) — no
  -- per-account cadence setting anymore (Section 5.1).
watchlist_companies
...
digests
  ...
  -- cadence_at_send removed alongside users.digest_cadence (0004_remove_cadence.sql)
price_history
...
| 1 | P0 | Watchlist CRUD + dashboard UI | Add/remove tickers from the live URL; data persists in Supabase on reload |
...
**Status: Phases 0-7 complete**, plus scope explicitly within the P0-P1 functional requirements that shipped after the initial pass: name search (6.1), 52-week high/low and 200-day MA triggers (5.1/5.2), Stock Spotlight and the watchlist summary section (6.4), digest preview and failure alerts (6.5), multi-user auth (6.6, Phase 7), removal of the per-account cadence setting in favor of one fixed daily schedule for everyone (5.1, `supabase/migrations/0004_remove_cadence.sql`), and an automated test suite for Section 10 (`npm test`, `lib/__tests__/`) replacing the manual spot-checks the acceptance checks above originally called for.
## 10. Test Cases
...
- **Dashboard:** one page. Ticker input, plain list, login/logout. No widgets, no charts, no settings — the digest schedule is fixed and not user-configurable (Section 5.1).
